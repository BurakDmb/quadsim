...
class LQG:
    def __init__(self, env):
        ...
        self.Q = np.eye(self.env.dynamics_len) * (self.env.noise_w_variance if hasattr(self.env, 'noise_w_variance') else 1)
        self.R = np.eye(self.env.dynamics_len) * (self.env.noise_v_variance if hasattr(self.env, 'noise_v_variance') else 1)
    ...
    def kalman_filter(self, y_k, u_k):
        ...
        S_k = self.C.dot(self.sigma_k_given_kminusone).dot(self.C.transpose()) + self.H.dot(self.R).dot(self.H.transpose())
        K_k = self.sigma_k_given_kminusone.dot(self.C.transpose()).dot(np.linalg.inv(S_k))

        self.x_hat_k_given_k = self.x_hat_k_given_kminusone + K_k.dot((y_k.reshape(-1,1) - y_hat_k_given_kminusone))
        self.sigma_k_given_k = self.sigma_k_given_kminusone - K_k.dot(S_k).dot(K_k.transpose())

        self.x_hat_k_given_kminusone = self.A.dot(self.x_hat_k_given_k) + self.B.dot(u_k)
        self.sigma_k_given_kminusone = self.A.dot(self.sigma_k_given_k).dot(self.A.transpose()) + self.G.dot(self.Q).dot(self.G.transpose())
